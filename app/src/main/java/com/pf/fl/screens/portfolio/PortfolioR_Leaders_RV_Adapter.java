...
import com.pf.shared.analyze.FLAnalyze;
...
public class PortfolioR_Leaders_RV_Adapter extends RecyclerView.Adapter<PortfolioR_Leaders_RV_Holder> {
    private List<FLAnalyze.FundRank> _leaders = new ArrayList<>();
    private Activity _parentActivity;
    private FLAnalyze _flAnalyze;

    public PortfolioR_Leaders_RV_Adapter(Activity parentActivity) {
        _parentActivity = parentActivity;
        List<D_FundInfo> fundsToAnalyze = DB_FundInfo_UI.getFundsForPortfolio(FLSingleton._portfolioName);
        _flAnalyze = new FLAnalyze(fundsToAnalyze);
        _flAnalyze.setRange(4);  // 4 is also the default of the textfield
    ...
    public void onBindViewHolder(PortfolioR_Leaders_RV_Holder holder, int position) {
        FLAnalyze.FundRank fr = _leaders.get(position);
        ...
        holder._position.setText(String.format("%.2f", fr._rank) + "/" + String.valueOf(_leaders.size()));
